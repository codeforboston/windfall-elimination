...
/* *getAIMEFromEarningsSimple()** Calculates the beneficiary's AIME from their earnings records while estimating future years
 Must replace getAIMEFromEarnings for the site to be predictive and allow it to be of use to our primary demographic
 Requires three new parameters: lastRealEarningYear, finalWorkYear, and indexingYearAverage -
 	however, we should be able to get this information without changing the interface, as follows:
 	* lastRealEarningYear *can* be inferred from looking at the rawEarnings table, though it may be better, moving forward, to ask the user for this
 	* We can assume finalWorkYear to be the year before the user retires, which we collect for other functions
 	* indexingYearAverage can be also be derived by looking up indexingYear in future entries of avgWageIndexTable
...
function getAIMEFromEarningsSimple(rawEarnings, indexingYear, maxECTable, avgWageIndexTable, lastRealEarningYear, finalWorkYear, indexingYearAverage) {
  ...
  if (Array.isArray(maxECTable)) {
    earnings.forEach((earning) => {
      if (earning.year >= 1950 && earning.year >= indexingYear-39 && (earning.year < indexingYear+2 || earningsMap[earning.year] !== undefined)) {
        maximumMap[earning.year] = maxECTable.find(d => d.year === earning.year).maximumEarningsCreditable;
      }
    });
  }//throw new Error("") - but what would this error be?
  ...
  if (Array.isArray(earningsYears)) {
    earningsYears.forEach((earningsYear) => {
      if (earningsYear > lastRealEarningYear) continue;

      if (earningsYear !== indexingYear && earningsYear !== NaN && maximumMap[earningsYear] !== undefined && averageMap[earningsYear] !== undefined) {
        debugValidEarnings.push({
          'yr': earningsYear,
          'specialIndexingYear': false,
          'indexOnYear': indexingYear,
          ...
          'avgMapIndex': averageMap,
          'avgMapYr': averageMap[earningsYear],
          'validEarningCalc': Math.min(earningsMap[earningsYear], maximumMap[earningsYear]) * indexingYearAverage / averageMap[earningsYear]
        });
        // validEarnings.push(Math.min(earningsMap[earningsYear], maximumMap[earningsYear]) * averageMap[indexingYear] / averageMap[earningsYear])
      }
      else if (earningsYear === indexingYear && earningsYear !== NaN && maximumMap[earningsYear] !== undefined && averageMap[earningsYear] !== undefined) {
        //TODO(tdk): what is this if statement for exactly? check with the narrative.
         debugValidEarnings.push({
          'yr': earningsYear,
          'specialIndexingYear': true,
          'eMap':earningsMap[earningsYear],
          ...
          'avgMapYr': averageMap[earningsYear],
          'validEarningCalc': Math.min(earningsMap[earningsYear], maximumMap[earningsYear])// * averageMap[indexingYear] / averageMap[earningsYear]
           // Commented out last part - this quantity should always be 1
        });
    ...
    for (int i = lastRealEarningYear+1; i < finalWorkYear+1; i++) {
      debugValidEarnings.push({
        ...
        'validEarningCalc':Math.min(earningsMap[lastRealEarningYear], maximumMap[lastRealEarningYear]) * 
        averageMap[lastRealEarningYear] / indexingYearAverage
      });
    }
  }
