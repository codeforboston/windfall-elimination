...
export function send(event, options) {
    return {
        ...
        id: options && options.id !== undefined ? options.id : typeof event === 'function' ? event.name : getEventType(event)
    };
}
export function resolveSend(action, ctx, event) {
    // TODO: helper function for resolving Expr
    var resolvedEvent = typeof action.event === 'function' ? toEventObject(action.event(ctx, event)) : toEventObject(action.event);
    var resolvedDelay = typeof action.delay === 'function' ? action.delay(ctx, event) : action.delay;
